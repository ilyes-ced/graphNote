...
export default (node: TableProps) => {
		...
		return (
			<Switch>
				<Match when={colType.typeDef === ColumnType.String}>
					<div class="px-2 py-1 bg-red-400 size-full" contentEditable>
						{value}
					</div>
				</Match>
	...
	const BadgeComponent = (props: { type: string; text: string; color: string }) => {
		return (
			<div
				onClick={(e) => {
...
const Table = (props: { children: JSX.Element }) => {
	return <table class="overflow-x-auto w-full text-sm text-left rtl:text-right text-foreground border border-border">{props.children}</table>
}
...
const TableHead = (props: { children: JSX.Element }) => {
	return <thead class="text-xs uppercase bg-background text-foreground">{props.children}</thead>
}
const TableHeaderRow = (props: { children: JSX.Element }) => {
	return <tr class="px-2 py-1 font-medium whitespace-nowrap text-foreground divide-x-1 divide-border">{props.children}</tr>
}
...
const TableCellButton = (props: { children: JSX.Element }) => {
	return <td class="px-2 py-1 cursor-pointer hover:bg-red-400">{props.children}</td>
}
	...
	return (
		<div class="flex flex-row items-center justify-between pt-2">
			<span class="text-md font-normal  text-foreground mb-4 md:mb-0 block w-full md:inline md:w-auto">
				Showing <span class="font-semibold text-primary">1-10</span> of <span class="font-semibold text-primary">1000</span>
			</span>

			<div class="flex divide-x-1 divide-border">
				<button class="cursor-pointer hover:bg-muted border-y border-l border-border px-4">first</button>
				...
				<button class="cursor-pointer hover:bg-muted border-y border-r border-border px-4">last</button>
			</div>
		</div>
	)
}
