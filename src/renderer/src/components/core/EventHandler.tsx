...
function extractImageReference(text: string): ImageReference {
	...
	const IMA_URL_REGEX = /(https?:\/\/[^\s"'<>]+?\.(?:png|jpg|jpeg|gif|webp|svg|bmp|avif|tiff))(?:\?[^\s"'<>]*)?/i

	const LOCAL_PATH_REGEX = /^(\/(?:[^\/]+\/)*[^\/]+\.(png|jpg|jpeg|gif|webp|bmp|svg))$/i
		...
		>
			...
			>
				<div
					...
					class="bg-background absolute size-fit border-2 border-primary animate-[modalIn_0.25s_ease]"
					style={{
						top: `${modalPos().y}px`,
						left: `${modalPos().x}px`
					}}
			...
			<Show when={store.contextMenuModal}>
				...
				>
					<div class="border border-border bg-card p-2 shadow-xl">
						{/* Normal item */}
						<button class="w-full px-3 py-2 text-left hover:bg-background transition-colors">Test</button>
						...
						<div class="relative group">
							<button class="flex w-full items-center justify-between px-3 py-2 hover:bg-background transition-colors">
          ...
          "
							>
								...
								>
									<div class="aspect-video w-10" style={{ background: getBoardBgColor() }} />

									<div class="flex items-center">Set Board Background Color</div>
								</button>
								...
								>
									<div class="aspect-video w-10" style={{ background: getBoardGridColor() }} />

									<div class="flex items-center">Set Board Grid Color</div>
								</button>
								...
								>
									...
									<div class="flex items-center">Set Board Background Image</div>
								</button>
								...
								>
									...
									<div class="flex items-center">Set Board Grid to Grid</div>
								</button>
								...
								>
									...
									<div class="flex items-center">Set Board Grid to dots</div>
								</button>
							</div>
						</div>
						{globalOptions()}
...
const Option = (text: string) => {
	return (
		<button class="w-full px-3 py-2 text-left hover:bg-background flex items-stretch gap-3 cursor-pointer" onclick={() => {}}>
