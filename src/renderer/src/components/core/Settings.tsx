			...
			<Portal>
				...
				>
					<div
						...
						class="bg-background absolute w-5xl aspect-video top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 border-2 border-primary animate-[modalIn_0.25s_ease]"
					>
						<div class="flex flex-col w-full">
							<div class="text-4xl p-4">category title</div>
							<div>
								<For each={ToggleSettings}>
									{(Setting) => (
										<SettingsItemToggle
											...
											value={store.userConfig[Setting.storeFieldName]}
											storeFieldName={Setting.storeFieldName}
										/>
									)}
								</For>
		...
		<div class="w-full px-4 py-1 space-y-4 text-white">
			<div class="flex items-center justify-between border border-border p-4 hover:bg-card transition">
				<div>
					...
					<div class="text-sm text-muted-foreground mt-1">{props.description}</div>
				</div>
				<Toggle
					checked={accepted()}
					onChange={() => {
		...
		<div class="w-full px-4 py-1 space-y-4 text-white">
			<div class="flex items-center justify-between border border-border p-4 space-y-3 hover:bg-card transition">
				...
				<div class="flex gap-4">
					<RadioGroup value={selected()} class="flex flex-row gap-4">
