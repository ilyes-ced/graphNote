...
export default (props: any) => {
	...
	const handleWheel = async (e: WheelEvent) => {
		...
		const currentIndex = ZOOM_LEVELS.reduce((closest, z, i) => {
			return Math.abs(z - store.viewport.scale) < Math.abs(ZOOM_LEVELS[closest] - store.viewport.scale) ? i : closest
		}, 0)

		let zoomIndex = currentIndex + (e.deltaY > 0 ? -1 : 1)

		zoomIndex = Math.max(0, Math.min(ZOOM_LEVELS.length - 1, zoomIndex))
