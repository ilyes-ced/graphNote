...
export default (node: nodeProps) => {
	...
	const { startDrag } = useDraggable(node.node, node.isChildNode, ignoredClasses(node.node.type))

	//? if node is color than its half the width because it looks niced small
	const initWidth =
		node.node.type === NodeType.Color ? 150 : (store.nodes[getActiveBoardId()].find((n) => n.id === node.node.id)?.width ?? undefined)
